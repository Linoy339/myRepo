...
export default function ParticipantList({
  ...
  const updateVerifiedStatus = (id) => {
    const indexNum = participants.findIndex((obj) => obj.id === id)
    let data = participants
    data[indexNum].isVerified = true
  ...
  return (
    <React.Fragment>
            ...
            columns={[
              {
                ...
                render: (x) => (
                  <div>
                    ...
                    {!x?.isVerified && <Icon className={classes.notverified}>remove_moderator</Icon>}
                  </div>
                ),
              },
              {
              ...
              },
              {
                ...
                searchable: false,
                render: (rowData) =>
                  participants[rowData.tableData.id]?.isVerified && (
              ...
              },
              {
                ...
                searchable: false,
                render: (rowData) =>
                  participants[rowData.tableData.id].consent && (
      ...
      <ResponsiveDialog fullScreen transient={false} animate open={!!verifyDialog}>
        <AppBar position="static" style={{ background: "#FFF", boxShadow: "none" }}>
          <Toolbar className={classes.toolbardashboard}>
            <IconButton onClick={() => setVerifyDialog(false)} color="default" aria-label="Menu">
              <Icon>arrow_back</Icon>
            </IconButton>
            <Typography variant="h5">
              {t("Verification")} {participant?.id ?? ""}
            </Typography>
          </Toolbar>
        </AppBar>
        <Divider />
        <Box py={8} px={4}>
          <PatientView
            ...
            participant={participant}
            onClose={(status) => {
              if (status) updateVerifiedStatus(participant?.id)
